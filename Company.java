...
/* removed to test new delete location function 
...
*/  
  ...
  public void deleteLocation() {
    System.out.println("");
    System.out.println("Location Deletion Interface");
    System.out.println("Please select a location below to be deleted.");
    int i = 0;
    for (Location indivLocation : locations ) {
      System.out.println(i + ": " + indivLocation.getName());
      i++;
    }//end for each loop print all locations
    Scanner locationDeleteScan = new Scanner(System.in);
    if (locationDeleteScan.hasNextInt()) {
      int locationDeleteIndex = locationDeleteScan.nextInt();
      Location tempLocation = locations.get(locationDeleteIndex);
      System.out.println("Location: " + tempLocation.getName() + " will be deleted. Proceed? (Y/N)");
      Scanner locationDeleteFinalScan = new Scanner(System.in);
      if (locationDeleteFinalScan.hasNextLine()) {
        String locationDeleteFinalStr = locationDeleteFinalScan.nextLine();
        if (locationDeleteFinalStr.toUpperCase().equals("Y")) {
          System.out.println("Location Deleted");
          locations.remove(locationDeleteIndex);
        } else if (locationDeleteFinalStr.toUpperCase().equals("N")) {
          System.out.println("Location will not be deleted.");
        } else {
          System.out.println("I'm sorry, that is not a recognized input. Please try again.");
        }//end final delete string conditionals
      } else {
        System.out.println("I'm sorry, that is not a recognized input. Please try again.");
      }//end final delete string check
    } else {
      System.out.println("I'm sorry, that is not a recognized input. Please try again.");
    }//end location delete integer check
  }//end deleteLocation
